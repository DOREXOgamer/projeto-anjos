...
router.get("/", requireAuth, async (req: AuthRequest, res) => {
  ...
  if (req.user!.role === Role.TEACHER) {
    ...
    const teacherStudents = studentsList.filter(s => 
    ...
    presentesHoje = attendancesList.filter(a => 
      a.date === todayStr && (a.status === "presente" || a.status === "PRESENT") && classIds.includes(a.class_id)
    ).length

    aulasDoDia = lessonsList.filter(l => 
      l.data === todayStr && classIds.includes(l.class_id)
    ).length
  } else {
    totalAlunos = studentsList.length
    presentesHoje = attendancesList.filter(a => 
      a.date === todayStr && (a.status === "presente" || a.status === "PRESENT")
    ).length
...
router.get("/students", requireAuth, async (req: AuthRequest, res) => {
  ...
  const newRegistrations7d = filteredStudents.filter(s => 
  ...
  const recentStudents = filteredStudents.slice(0, 10).map((s: any) => ({
    id: s.id,
    name: s.nome,
